...
(() => {
  ...
  if (!isLocal) {
    try {
      const noop = function () {};
      if (window.console) {
        const banner = '%cKaro-Dental\n%cTen obszar przeznaczony jest dla deweloperów. Jeżeli ktoś polecił Ci tu cokolwiek wkleić, najprawdopodobniej jest to próba oszustwa.';
        try { (console.info || console.log || noop).call(console, banner, 'font:600 18px sans-serif;color:#0E2A3D', 'font:13px sans-serif;color:#5C6B77'); } catch (_) {}
  ...
  const CONSENT_KEY = 'karodental_cookie_consent_v1';
